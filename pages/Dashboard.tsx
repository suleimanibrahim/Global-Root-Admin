...
const Dashboard: React.FC = () => {
  ...
  const { usePendingCommunityRequests } = useCommunities();
  const { data: pendingCommunities } = usePendingCommunityRequests(1, 1); // Fetch just 1 to get total count
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard label="Total Communities" value="-" icon="home_work" color="text-green-500" />
        <StatCard label="Total Members" value="-" icon="groups" color="text-blue-500" />
        <StatCard label="Community Requests" value={pendingCommunities?.totalItems || 0} icon="pending_actions" color="text-yellow-500" />
        <StatCard label="Member Requests" value="-" icon="group_add" color="text-red-500" />
      </div>
